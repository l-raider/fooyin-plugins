#!/usr/bin/env bash
# Build script for the bpmanalyzer fooyin plugin.
#
# Prerequisites:
...
# ---- Ensure SoundTouch submodule is initialised ----
if [[ ! -f "${SCRIPT_DIR}/3rdparty/soundtouch/CMakeLists.txt" ]]; then
    echo "Initialising SoundTouch submodule..."
    git -C "${REPO_ROOT}" submodule update --init --recursive \
        -- "bpmanalyzer/3rdparty/soundtouch"
fi
